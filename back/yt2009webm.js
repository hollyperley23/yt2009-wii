    ...
    "vid_webm": function(id, callback) {
        ...
        if(fs.existsSync(`../assets/${id}.webm`)) {
            callback()
                ...
                && !fs.existsSync(`../assets/${id}.webm`)) {
            // have mp4 but no webm, convert
            convert_mp4_to_webm(id, () => {
                callback()
            })
        } else {
        ...
        function convert_mp4_to_webm(id, callback) {
            console.log("WebM process start")
            webmProcessingVideos.push(id)
            child_process.exec(ffmpegCommandWebm.join(" "),
            (error, stdout, stderr) => {
                console.log("WebM process done, remove from list")
